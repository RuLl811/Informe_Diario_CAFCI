...
basedia["fecha"] = pd.to_datetime(fecha_informe, format='%Y/%m/%d')
...
########################################################################################################################
start_time = time.time()
def obtener_datos_por_fecha(date_string):
  date = pd.to_datetime(date_string).date()
  datos_filtrados = concat_princip_basedia[(pd.to_datetime(concat_princip_basedia["fecha"]).dt.date == date)]
  datos_filtrados = datos_filtrados[["clase_id", "compute_0013", "patrimonio", "fecha"]]
  datos_filtrados.reset_index(drop=True, inplace=True)
  return datos_filtrados

# Particiono el dataframe en dos días
estimacion_vcp_0 = obtener_datos_por_fecha(fecha_informe)
estimacion_vcp_1 = obtener_datos_por_fecha(undia)

# Genero un subdataframe de equivalencias
estimacion_equivalencias = pd.DataFrame(equivalencias[["clase_id", "segmento_cajon", "clasi_nombre"]])

# Supongamos que tienes los DataFrames estimacion_principal y estimacion_vcp_0

base_est_vcp = pd.merge(estimacion_equivalencias, estimacion_vcp_0, on='clase_id', how='left')
base_est_vcp = pd.merge(base_est_vcp, estimacion_vcp_1, on='clase_id', how='left', suffixes=('_daily', '_undia'))
base_est_vcp = base_est_vcp.rename(columns={'compute_0013_daily': 'VCP_daily', 'compute_0013_undia': 'VCP_undia',
                                            'patrimonio_undia': 'AUM_udia', 'patrimonio_daily': 'AUM_daily', 'fecha_daily': 'fecha'})  # Renombro las columnas de VCP para arreglar el sufix
base_est_vcp = base_est_vcp.fillna(0)
...
# Matriz con las clases que presentaron. La uso para calcular el rto ponderado SIN las clases que no presentaron
presentaron_vcp = base_est_vcp.copy()  # Creo una copia para evitar el warning
presentaron_vcp = presentaron_vcp[~presentaron_vcp['clase_id'].isin(condiciones)]  # Aplico la condicion para que solo tengo las clases que presetaron

presentaron_vcp["rend_daily"] = (presentaron_vcp['VCP_daily']/presentaron_vcp['VCP_undia']) - 1
rendimiento_ponderado_cajon_daily = presentaron_vcp.groupby('segmento_cajon').apply(lambda x: ((x['rend_daily'] * x['AUM_daily']).sum() / x['AUM_daily'].sum()) if x['AUM_daily'].sum() != 0 else 0).reset_index()
rendimiento_ponderado_cajon_daily.columns = ['segmento_cajon', 'Rto_ponderado_daily']

#  Genero una matriz copia que contenga las clases que NO presentaron y luego calculo el VCP estimado en base al rto ponderado del cajon
no_presetaron_vcp = base_est_vcp.loc[base_est_vcp['clase_id'].isin(condiciones)]
no_presetaron_vcp = pd.merge(no_presetaron_vcp, rendimiento_ponderado_cajon_daily,  on='segmento_cajon', how='left')
no_presetaron_vcp['VCP_daily'] = no_presetaron_vcp['VCP_undia'] * (1 + no_presetaron_vcp['Rto_ponderado_daily'])
no_presetaron_vcp = no_presetaron_vcp[['clase_id', 'VCP_daily']]
no_presetaron_vcp["fecha"] = fecha_informe
no_presetaron_vcp["fecha"] = pd.to_datetime(no_presetaron_vcp['fecha'], format='%Y/%m/%d')

# Actualizo la el principal
df_merged = pd.merge(concat_princip_basedia, no_presetaron_vcp, on=['clase_id', 'fecha'], how='left')
df_merged['compute_0013'] = np.where(df_merged['VCP_daily'].notnull(), df_merged['VCP_daily'], df_merged['compute_0013'])
concat_princip_basedia = df_merged.drop('VCP_daily', axis=1)

end_time = time.time()
total_time5 = end_time - start_time
print("Tiempo total de estimación de VCP no presentados: ", round(total_time5, 4), "segundos")
...
principal_daily = obtener_datos_por_fecha(fecha_informe)
principal_undia = obtener_datos_por_fecha(undia)
principal_mtd = obtener_datos_por_fecha(Month_to_date)
principal_30_DAYS = obtener_datos_por_fecha(treintadias)
principal_3Month = obtener_datos_por_fecha(noventadias)
principal_6Month = obtener_datos_por_fecha(cientoochentadias)
principal_YTD = obtener_datos_por_fecha(Year_to_date)
principal_12Month = obtener_datos_por_fecha(tressesentadias)

end_time = time.time()
total_time6 = end_time - start_time
print("Tiempo total de procesamiento particiones principal: ", round(total_time6, 4), "segundos")
...
print("Tiempo total de procesamiento Benchmark: ", round(total_time7, 4), "segundos")
...
# Calculo las var rel DTI en base al fecha informe
...
print("Tiempo total de procesamiento BNA/DTI: ", round(total_time8, 4), "segundos")

########################################################################################################################
#############################           MATRICES AUM Y RENDIMIENTOS         ############################################
########################################################################################################################

#############################             MATRIZ MONEDA ORIGINAL            ############################################
...
principal_vcp_daily = pd.DataFrame(principal_daily[['clase_id', 'compute_0013']])
...
matriz = pd.merge(matriz, principal_vcp_daily, on='clase_id', how='left').rename(columns={'compute_0013': 'VCP_daily'})
...
print("Tiempo total de procesamiento Matriz MO: ", round(total_time9, 4), "segundos")
...
print("Tiempo total de procesamiento Matriz ARS: ", round(total_time10, 4), "segundos")
...
print("Tiempo total de procesamiento Fee & Calificaciones: ", round(total_time11, 4), "segundos")
...
print("Tiempo total de procesamiento Evol. Patr.: ", round(total_time12, 4), "segundos")
...
print("Tiempo total de procesamiento AUM por fondo: ", round(total_time13, 4), "segundos")
########################################################################################################################
#############################                INDUSTRIA FCI-AUM            ##############################################
########################################################################################################################
start_time = time.time()
...
print("Tiempo total de procesamiento Industria FCI-AUM: ", round(total_time14, 4), "segundos")
...
print("Tiempo total de procesamiento MS: ", round(total_time15, 4), "segundos")
...
print("Tiempo total de procesamiento Rto. Ponderado: ", round(total_time16, 4), "segundos")
...
print("Tiempo total de procesamiento Ventas Netas: ", round(total_time17, 4), "segundos")
########################################################################################################################
#############################                EXPORTACION A EXCEL            ############################################
########################################################################################################################
start_time = time.time()
...
writer = pd.ExcelWriter(fr'C:\Users\lr110574\PycharmProjects\Informe_Diario\Salidas\Matriz de Salida {fecha_informe}.xlsx', engine='xlsxwriter')
...
shutil.copyfile(fr'C:\Users\lr110574\PycharmProjects\Informe_Diario\Salidas\Matriz de Salida {fecha_informe}.xlsx', 'Matriz de Salida.xlsx')
...
end_time = time.time()
total_time18 = end_time - start_time
print("Tiempo total de procesamiento exportaciones: ", round(total_time18, 4), "segundos")
